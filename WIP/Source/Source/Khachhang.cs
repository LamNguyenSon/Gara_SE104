...
namespace Source.Resources
{
    public partial class Khachhang : Form
    {
        ...
        private void cmdThem_Click(object sender, EventArgs e)
        {
            ...
            {
                DATAPROVIDER kn = new DATAPROVIDER();
                int kq = kn.Executenonquery("insert into KHACHHANG(MAKH,HOTEN,CMND,NGAYSINH,GIOITINH,DIACHI,SDT,BIENSO,LOAIXE) values('" + txtMaKH.Text + "','" + txtHoTen.Text + "','" + txtCMND.Text + "','" + dtNgaySinh.Text + "','" + coboGioiTinh.Text + "','" + txtDiaChi.Text + "','" + txtSDT.Text + "','" + txtBienSo.Text + "','" + txtLoaiXe.Text + "')");
                if (kq > 0)
                {
        ...
        public void LoadListByKeyWord()
        {
            DATAPROVIDER kn = new DATAPROVIDER();
            DataTable dt = new DataTable();
            dt = kn.Executequery("select * from KHACHHANG where HOTEN like'%" + txtTimKiem.Text.ToString() + "%' ");
            dataGridKhachHang.DataSource = dt;
        }
